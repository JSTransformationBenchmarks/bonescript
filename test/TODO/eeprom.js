  ...
  if (!module.parent) {
    ...
    if (process.argv.length > 0 && process.argv[0].match(/^-w/i)) {
      ...
      try {
        ...
        var jsonFile = await fs_readFilePromise(sourceJSON, 'ascii');
    ...
    } else if (process.argv.length === 0 || process.argv.length > 0 && process.argv[0].match(/^-r/i)) {
      // Read EEPROMs
